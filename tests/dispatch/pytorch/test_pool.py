...
def rng():
    return np.random.default_rng(sum(map(ord, "torch pool")))
...
def test_the_pool_op_matches_the_graph_it_replaces(layer_cls, rng):
    ...
    assert_close(
        np.asarray(pytensor.function([X], out, mode="PYTORCH")(X_np)),
        out.eval({X: X_np}),
    )
...
def test_max_pooling_conserves_the_gradient_of_a_tied_window():
    ...
    dispatched = pytensor.function([X], gradient, mode="PYTORCH")(X_np)
