...
def rng():
    return np.random.default_rng(sum(map(ord, "mlx pool")))
...
def test_the_pool_op_matches_the_graph_it_replaces(layer_cls, rng):
    ...
    compare_mlx_and_py([X], out, [X_np], assert_fn=assert_close)
...
def test_the_pool_gradient_dispatch_matches_the_graph(layer_cls, rng):
    ...
    compare_mlx_and_py([X], gradient, [X_np], assert_fn=assert_close)
...
def test_max_pooling_conserves_the_gradient_of_a_tied_window():
    ...
    dispatched = pytensor.function([X], gradient, mode="MLX")(X_np)
